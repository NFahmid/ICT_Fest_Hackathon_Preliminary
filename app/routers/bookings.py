...
# Serializes the conflict-check → quota-check → insert critical section so that
# concurrent creates cannot both pass their checks and commit overlapping or
# over-quota bookings.
...
def cancel_booking(
    ...
    # Atomically claim the cancellation: only the request that flips the row
    # from "confirmed" to "cancelled" proceeds. Concurrent or repeat cancels
    # match zero rows and get 409 ALREADY_CANCELLED, so exactly one RefundLog
    # is ever written for a booking.
    claimed = (
        db.query(Booking)
